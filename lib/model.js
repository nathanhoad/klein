...
class Model {
    constructor (table_name, args) {
        ...
        if (args.default_scope) {
            ...
            this.query = this.knex.select().from(this.table_name);
        }
        ...
        return new Promise((resolve, reject) => {
            this.knex(this.table_name).where({ id: model.get('id') }).del().then(() => {
                ...
                Promise.all(dependent_relations.map(dependent_relation => {
                    ...
                    return this.knex(table).where(key, model.get('id')).del();
                })).then(() => {
                    resolve(model);
                }).catch(err => {
                    reject(err);
        ...
        return new Promise((resolve, reject) => {
            ...
            Promise.resolve().then(() => {
                ...
                if (options.exists === true) {
                    ...
                    return this.knex(this.table_name).where({ id: properties.id || null }).then(rows => {
                ...
                if (exists) {
                    return this.knex(this.table_name).where({ id: properties.id }).update(properties, '*');
                } else {
                    return this.knex.insert(properties, '*').into(this.table_name);
                }
            }).then(results => {
                ...
                return this._saveRelations(object);
    ...
    reload (model) {
        return this.where({ id: model.get('id') }).first();
    }
    ...
    first () {
        return new Promise((resolve, reject) => {
            this.query.then(results => {
                ...
                this._includeRelations(results).then(results => {
    ...
    */
        ...
        return new Promise((resolve, reject) => {
            ...
            Promise.all(properties_that_are_relations.map(property_name => {
                ...
                switch (relation.type) {
                    case 'has_many': return this._saveHasManyRelation(model, relation, property_value);
                    case 'belongs_to': return this._saveBelongsToRelation(model, relation, property_value);
                    case 'has_and_belongs_to_many': return this._saveHasAndBelongsToManyRelation(model, relation, property_value);
    ...
    _saveHasManyRelation (model, relation, related_objects) {
        return new Promise((resolve, reject) => {
            ...
            this.knex(relation.table).where(relation.key, model.id).whereNotIn('id', new_related_objects_ids).update({ [relation.key]: null }).then(() => {
                // Find any related objects that are already in the database
                this.knex(relation.table).select('id').whereIn('id', new_related_objects_ids).then(existing_related_ids => {
                    Promise.all(related_objects.map(related_object => {
                        return new Promise((resolveRelated, rejectRelated) => {
                            ...
                            Promise.resolve().then(() => {
                                // Check if this object needs to be created or just updated
                                if (new_related_objects_ids.includes(related_object.id)) {
                                    return this.knex(relation.table).where({ id: related_object.id }).update(related_object, '*');
                                } else {
                                    related_object = Object.assign({}, {
                                        id: uuid(),
                                        created_at: new Date(),
                                        updated_at: new Date()
                                    }, related_object);
                                    return this.knex.insert(related_object, '*').into(relation.table);
                                }
                            }).then(related_object => {
                                if (related_object instanceof Array) related_object = Object.assign({}, related_object[0]);
    ...
    _saveBelongsToRelation (model, relation, related_object) {
        return new Promise((resolve, reject) => {
            model = model.toJS ? model.toJS() : model;
            
            // see if the related object already exists
            this.knex(relation.table).select('id').where({ id: related_object.id || null }).then(existing_related_objects => {
                if (existing_related_objects.length == 0) {
                    ...
                    return this.knex(relation.table).insert(related_object, '*');
                    
                } else {
                    // Update the already saved object
                    related_object.updated_at = new Date();
                    return this.knex(relation.table).where({ id: related_object.id }).update(related_object, '*');
                }
                
            }).then(object => {
                if (object instanceof Array) object = Object.assign({}, object[0]);
                
                // Update the related field on this model
                this.knex(this.table_name).where({ id: model.id }).update({ [relation.key]: object.id }).then(() => {
                    resolve({
                        name: relation.name,
                        value: object,
                        // Return with the information to update the current model
                        belongs_to_key: relation.key,
                        belongs_to_value: object.id
                    });
                }).catch(err => {
                    reject(err);
                });
    ...
    _saveHasAndBelongsToManyRelation (model, relation, related_objects) {
        return new Promise((resolve, reject) => {
            model = model.toJS ? model.toJS() : model;
            ...
            this.knex(relation.through_table).select(relation.key).where(relation.source_key, model.id).then(existing_related_object_ids => {
                ...
                this.knex(relation.through_table).where(relation.source_key, model.id).whereNotIn(relation.key, new_relation_ids).del().then(() => {
                    // Work out which related rows already exist
                    // eg. which projects already exist
                    this.knex(relation.table).select('id').whereIn('id', new_relation_ids).then(existing_related_ids => {
                        ...
                        Promise.all(related_objects.map(related_object => {
                            return new Promise((resolveRelated, rejectRelated) => {
                                ...
                                Promise.resolve().then(() => {
                                    if (existing_related_ids.includes(related_object.id)) {
                                        return this.knex(relation.table).where({ id: related_object.id }).update(related_object, '*');
                                    } else {
                                        related_object = Object.assign({}, {
                                            id: uuid(),
                                    ...
                                    // Create the join row unless it exists
                                    // eg. create the projects_users record for this project and user unless it exists
                                    if (existing_related_object_ids.includes(object.id)) {
                                        return Promise.resolve(object);
                                    } else {
                                        const new_join_row = {
                                            id: uuid(),
                                            [relation.key]: related_object.id,
                                            [relation.source_key]: model.id,
                                            updated_at: new Date(),
                                            created_at: new Date()
                                        }
                                        
                                        return this.knex(relation.through_table).insert(new_join_row, 'id').then(() => {
                                            return object;
                                        });
                                    }
                                }).then(related_object => {
            ...
            Promise.all(this._included_relations.map(relation_name => {
                return new Promise((resolve, reject) => {
                    ...
                    if (relation.type === 'has_many') {
                        ...
                        */
                        this.knex(relation.table).select('*').whereIn(relation.key, ids).then(related_rows => {
                        ...
                        */
                        this.knex(relation.through_table).select(relation.source_key, relation.key).whereIn(relation.source_key, ids).then(through_ids => {
                            let joins = through_ids.map(r => Object.assign({}, r));
                            through_ids = through_ids.map(r => r[relation.key]);
                            this.knex(relation.table).select('*').whereIn('id', through_ids).then(related_rows => {
                                related_rows = related_rows.map(r => Object.assign({}, r));
                        ...
                        this.knex(relation.table).select('*').whereIn(relation.key, ids).limit(1).then(related_rows => {
                            related_rows = related_rows.map(r => Object.assign({}, r));
                            return resolve({ name: relation_name, properties: relation, rows: related_rows });
                        }).catch(err => {
                            reject(err);
                        ...
                        */
                        ...
                        this.knex(relation.table).select('*').whereIn('id', relation_ids).then(related_rows => {
                            related_rows = related_rows.map(r => Object.assign({}, r));
                            return resolve({ name: relation_name, properties: relation, rows: related_rows });
                        }).catch(err => {
                            reject(err);
