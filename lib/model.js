...
class Model {
    constructor (table_name, args) {
        ...
        if (args.default_scope) {
            ...
            this.query = this.knex(this.table_name).select();
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            Promise.resolve().then(() => {
                ...
                if (exists) {
                    ...
                    return this.knex(this.table_name).insert(properties, '*');
                }
            }).then(results => {
            ...
            this.knex(relation.table).where(relation.key, model.id).whereNotIn('id', new_related_objects_ids).update({ [relation.key]: null }).then(() => {
                ...
                this.knex(relation.table).select('id').whereIn('id', new_related_objects_ids).then(existing_related_ids => {
                    Promise.all(related_objects.map(related_object => {
                        ...
                        return RelatedModel.save(related_object, { exists: new_related_objects_ids.includes(related_object.id) });
    ...
    _saveBelongsToRelation (model, relation, related_object) {
        return new Promise((resolve, reject) => {
            ...
            RelatedModel.save(related_object).then(object => {
                this.knex(this.table_name).where({ id: model.id }).update({ [relation.key]: object.get('id') }).then(() => {
                    resolve({
                        name: relation.name,
                        value: object,
                    ...
                    this.knex(relation.table).select('id').whereIn('id', new_relation_ids).then(existing_related_ids => {
                        ...
                        Promise.all(related_objects.map(related_object => {
                            return new Promise((resolveRelated, rejectRelated) => {
                                ...
                                // Promise.resolve().then(() => {
                                RelatedModel.save(related_object, { exists: existing_related_ids.includes(related_object.id) }).then(object => {
